...
Script: ~/code/scripts/notion_task.py
...
2. **Find the project** — only if the user mentions a project (3 API calls):
   ```bash
   python3 ~/code/scripts/notion_task.py search-project "query term"
   ```
...
3. **Create the task** (2 API calls):
   ```bash
   python3 ~/code/scripts/notion_task.py create '<json>'
   ```
...
- If env vars are missing, remind the user to add NOTION_TOKEN and NOTION_DATABASE_ID to ~/.claude_env.
