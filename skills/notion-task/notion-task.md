...
2. **Find the project** — only if the user mentions a project:
   ```bash
...
4. Confirm success: show the task title and the Notion URL from the result along with reported time and steps.

## Guidelines
- Always set **Task** and **Kanban** — these are the minimum required fields.
- Kanban reflects urgency/timing: map "today", "now", "urgent" → "Today"; "tomorrow" → "Tomorrow"; "this week" → "This week"; no time context → "This week".
...
- Always include the time it took to create the task and the intermediate steps you took to do so.
